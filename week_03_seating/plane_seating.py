...
#this code, unlike purchase_economy_plus, does not instinctively tryto grab
#the window seat
    ...
    while not(found_seat):
        r_row = random.randrange(0,rows)
        r_col = random.randrange(0,cols)
        #checking if seat you're looking at is avialable or not
...
def fill_plane(plane):
    ...
    # for example, when to stop purchases, the probabilities,
    # maybe the size for the random
    # regular economy size

############
#My goal for this algorithm was to prioritize people who pay a premium
#and people who need larger family units. I therefore increased the
#the max family size to 4 and decreased the chance someone would be seated into a window seats


    max_family_size = 4 #max family size changed to 4 to accomodate 2 pairs, possibly 2 parents, 2 kiddos
    #allow purchasing based on percent of seats left
    while total_seats > math.floor(total_seats*0.05): #while there are 5% of the seats left, we'll keep allowing purchases
        r = random.randrange(100)
        #probability of window seat lowered
        if r > 20: #20% chance that someone gets a window seat (i.e. economy plus)
